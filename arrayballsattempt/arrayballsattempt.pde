...
void setup() {
  ...
  for (int i=0; i<numBalls; i++) { // this loop initializes all the values using the random function
   ballX[i]=random(ball2Diam[i], width-ball2Diam[i]); // come in from the edge slightly...
    ballY[i]=random(ball2Diam[i], height-ball2Diam[i]); // come in from the edge slightly...
